...
def sharesFun(alloc_SR, endDate, total = 10000):
    stocks = alloc_SR.columns.values[1:].tolist()
    stockData = pdr.get_data_yahoo(stocks, start=endDate, end=endDate)
    price = stockData['Close'].values
    
    alloc_money = (alloc_SR.values[:,1:]/100*total).astype(int)
    
    shares = np.round(alloc_money/price)
    
    shares = pd.DataFrame(shares, columns = stocks)
    shares.insert(0, "Portfolio Shares", ["Max SR"], True)
...
shares_SR = sharesFun(alloc_SR, endDate, total = 10000)
...
print(shares_SR.to_markdown(index=False))
